...
def medalAldur(medAld):
    talnalisti = []
    for x in medAld:
        talnalisti.append(x)
        print("meðaltal talnana er =", round(sum(talnalisti) / len(talnalisti), 2))

def heilAldur(helAld):
    talnalisti1 = []
    for x in helAld:
        talnalisti1.append(x)
        print("x er ", x)
        print("dict x er ",helAld.appe[x])
        print("summa talan er =", sum(talnalisti1))

def stafAldur(aldMedal):
        aldListi = []
        for x in aldMedal:
            if aldMedal == x[0]:
                aldListi.append(x)
...
while val != 4:
    ...
    if val ==1:
        ...
        if svar.upper() == "A":
            ...
            baedi_tup(tuple_dansherra,tuple_domurnar)
            print()
        ...
        if svar.upper() == "A":
            allNema(nafnaldur_dict.keys())
            print()
        elif svar.upper() == "B":
            nemAldur(nafnaldur_dict.keys())

        elif svar.upper() == "C":
            medalAldur(nafnaldur_dict)

        elif svar.upper() == "D":
            heilAldur(nafnaldur_dict)

        elif svar.upper() == "E":
            staf = input("slaðu inn staf")
            print(stafAldur(nafnaldur_dict))
